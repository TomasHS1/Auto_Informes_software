...
export function DialogAPA({ open, onClose, onSave, initialArgs = {} }) {
  const [texto, setTexto] = useState(
    (initialArgs.lista_referencias || ["Referencia APA..."]).join("\n")
  );
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 z-50 focus:outline-none">
          <div className="flex items-center justify-between mb-4">
            <Dialog.Title className="text-lg font-bold text-[#1F2937]">Editar Bibliografia APA</Dialog.Title>
            <button onClick={onClose} className="p-1 rounded-lg hover:bg-gray-100"><X size={20} /></button>
          </div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Referencias (una por linea, formato APA)
          </label>
          <textarea
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            <button
              onClick={() => {
                ...
                onSave({ lista_referencias: refs.length > 0 ? refs : ["Referencia APA..."] });
              }}
              className="px-4 py-2 text-sm font-semibold text-white bg-[#2563EB] hover:bg-[#1D4ED8] rounded-lg"
            >
              Guardar
            </button>
          </div>
