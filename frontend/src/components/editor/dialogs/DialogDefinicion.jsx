...
export function DialogDefinicion({ open, onClose, onSave, initialArgs = {} }) {
  const [termino, setTermino] = useState(initialArgs.termino || "");
  const [descripcion, setDescripcion] = useState(initialArgs.descripcion || "");
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 z-50 focus:outline-none">
          <div className="flex items-center justify-between mb-4">
            <Dialog.Title className="text-lg font-bold text-[#1F2937]">Editar Definicion / Actor</Dialog.Title>
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            <button onClick={() => onSave({ termino, descripcion })} className="px-4 py-2 text-sm font-semibold text-white bg-[#2563EB] hover:bg-[#1D4ED8] rounded-lg">Guardar</button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
